<?php

declare(strict_types=1);

namespace Modules\Core\Services\LmStudio\DTO\Completion;

use Modules\Core\Services\LmStudio\DTO\DataTransferObject;
use Modules\Core\Services\LmStudio\DTO\Shared\Usage;

/**
 * @phpstan-type RawChoice array<string, mixed>
 */
final class CompletionResponse extends DataTransferObject
{
    /**
     * @param  array<int, CompletionChoice>  $choices
     */
    public function __construct(
        public readonly string $id,
        public readonly string $object,
        public readonly int $created,
        public readonly string $model,
        public readonly array $choices,
        public readonly ?Usage $usage = null,
        public readonly array $raw = [],
    ) {
        self::assertArrayOf(CompletionChoice::class, $choices, 'choices');
    }

    /**
     * @param  array<string, mixed>  $payload
     */
    public static function fromArray(array $payload): self
    {
        $choices = array_map(
            static fn (array $choice): CompletionChoice => CompletionChoice::fromArray($choice),
            $payload['choices'] ?? []
        );

        return new self(
            id: $payload['id'],
            object: $payload['object'] ?? 'text_completion',
            created: $payload['created'] ?? time(),
            model: $payload['model'] ?? '',
            choices: $choices,
            usage: isset($payload['usage']) ? Usage::fromArray($payload['usage']) : null,
            raw: $payload,
        );
    }

    public function toArray(): array
    {
        return [
            'id' => $this->id,
            'object' => $this->object,
            'created' => $this->created,
            'model' => $this->model,
            'choices' => array_map(
                static fn (CompletionChoice $choice): array => $choice->toArray(),
                $this->choices
            ),
            'usage' => $this->usage?->toArray(),
            'raw' => $this->raw,
        ];
    }
}
